...
typedef ssize_t (*firing_policy_create_from_payload_cb)(
		...
		struct lttng_firing_policy **firing_policy);
typedef struct lttng_firing_policy *(*firing_policy_duplicate_cb)(
...
struct lttng_firing_policy {
	...
	firing_policy_duplicate_cb duplicate;
};
		...
		firing_policy_duplicate_cb duplicate)
{
	...
	firing_policy->duplicate = duplicate;
}
...
{
	int ret;
	struct lttng_firing_policy_comm firing_policy_comm = {
...
/* Every N */
static struct lttng_firing_policy_every_n *
firing_policy_every_n_from_firing_policy(struct lttng_firing_policy *policy)
{
	assert(policy);

	return container_of(policy, struct lttng_firing_policy_every_n, parent);
}
		...
		struct lttng_payload *payload)
{
	...
	struct lttng_firing_policy_every_n *every_n_policy;
	struct lttng_firing_policy_every_n_comm comm = {};

	assert(policy);
	assert(payload);

	every_n_policy = firing_policy_every_n_from_firing_policy(policy);
...
static struct lttng_firing_policy *lttng_firing_policy_every_n_duplicate(
		const struct lttng_firing_policy *source)
{
	struct lttng_firing_policy *duplicate = NULL;
	...
	duplicate = lttng_firing_policy_every_n_create(
		...
		uint64_t interval)
{
	...
	lttng_firing_policy_init(&policy->parent,
			...
			lttng_firing_policy_every_n_duplicate);
...
static struct lttng_firing_policy_once_after_n *
firing_policy_once_after_n_from_firing_policy(
		struct lttng_firing_policy *policy)
{
	assert(policy);
		...
		struct lttng_payload *payload)
{
	...
	struct lttng_firing_policy_once_after_n *once_after_n_policy;
	...
	once_after_n_policy =
			firing_policy_once_after_n_from_firing_policy(policy);
...
static struct lttng_firing_policy *lttng_firing_policy_once_after_n_duplicate(
		const struct lttng_firing_policy *source)
{
	struct lttng_firing_policy *duplicate = NULL;
	...
	once_after_n_policy =
			firing_policy_once_after_n_from_firing_policy_const(
					source);
	duplicate = lttng_firing_policy_once_after_n_create(
		...
		uint64_t threshold)
{
	...
	lttng_firing_policy_init(&policy->parent,
			...
			lttng_firing_policy_once_after_n_duplicate);
...
LTTNG_HIDDEN
struct lttng_firing_policy *lttng_firing_policy_duplicate(
		const struct lttng_firing_policy *source)
{
	assert(source->duplicate);
	return source->duplicate(source);
}
