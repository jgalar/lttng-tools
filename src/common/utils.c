...
int utils_parse_duration_suffix(const char * const str, uint64_t * const size)
{
	...
	if (ret) {
		 ...
		 */
		value = atoll(&str[pmatch[1].rm_so]);
		/* No unit suffix, consider micro-seconds. */
		if (pmatch[2].rm_eo - pmatch[2].rm_so == 0) {
			*size = value;
		} else if (!strncmp(&str[pmatch[2].rm_so], "u\0", 2) ||
				!strncmp(&str[pmatch[2].rm_so], "us\0", 3)) {
			*size = value;
		} else if (!strncmp(&str[pmatch[2].rm_so], "ms\0", 3)) {
			*size = value * 1000;
		} else if (!strncmp(&str[pmatch[2].rm_so], "s\0", 2)) {
			*size = value * 1000 * 1000;
		} else if (!strncmp(&str[pmatch[2].rm_so], "m\0", 2)) {
			*size = value * 1000 * 1000 * 60;
		} else if (!strncmp(&str[pmatch[2].rm_so], "h\0", 2)) {
