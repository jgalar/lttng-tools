 ...
 */
static inline
size_t lttng_dynamic_array_get_size(const struct lttng_dynamic_array *array)
{
	return array->size;
}
 ...
 * is built under the audacious assumption that pointer sizes are equal
 ...
 */
static inline
size_t lttng_dynamic_pointer_array_get_size(
		const struct lttng_dynamic_pointer_array *array)
{
	return lttng_dynamic_array_get_size(&array->array);
}
		...
		const struct lttng_dynamic_pointer_array *array, size_t index)
{
	...
	return ((void **) array->array.buffer.data)[index];
}
